...
import { api, type WhItemDrill } from "../lib/api";
import { DataTable, type Column } from "./DataTable";
import { SeverityPill } from "./Pill";
import { fmtNum, availColor, severityColor } from "../lib/format";
...
export function WhItemDrillPanel({ wh, item, onClose }: { wh: string; item: string; onClose: () => void }) {
  const [data, setData] = useState<WhItemDrill | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setData(null);
    setError(null);
    api.whItemDrill(wh, item).then(setData).catch((e) => setError(e.message));
  }, [wh, item]);
      ...
      {data && (
        <>
          ...
          <div className="rounded-lg mt-2 mb-3 px-3 py-2.5 text-[12px]" style={{ background: "var(--surface-2)", color: "var(--text-secondary)" }}>
            This item runs at <b style={{ color: "var(--text-primary)" }}>{fmtNum(data.national_cpd)} CPD</b> nationally across{" "}
            <b style={{ color: "var(--text-primary)" }}>{data.national_wh_count}</b> warehouses; it is{" "}
            <b style={{ color: "var(--status-critical)" }}>out in {data.out_wh_count}</b> of them (
            {fmtNum(data.out_cpd)} CPD at risk).
          </div>

          <div className="grid gap-2.5 mb-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))" }}>
            <Tile value={fmtNum(data.cpd_here)} label="CPD here" sub={`${data.pct_of_national.toFixed(1)}% of item national`} accent="var(--series-1)" />
            ...
            <Tile value={data.doi.toFixed(1)} label="DOI" sub="days of cover" accent={severityColor(data.doi < 3 ? 100 : 0)} />
            <Tile value={fmtNum(data.open_po)} label="Open PO" sub="units inbound" accent="var(--series-4)" />
            <Tile value={fmtNum(data.pipeline)} label="Inv + PO" sub="if all lands" accent="var(--series-4)" />
            {data.fill_rate_available && (
              <>
                <Tile value={data.fill_L1 == null ? "–" : `${data.fill_L1.toFixed(0)}%`} label="Fill Rate · L1" sub="grn ÷ ordered" accent={data.fill_L1 == null ? "var(--text-muted)" : availColor(data.fill_L1)} />
          ...
          <h4 className="text-[12.5px] font-semibold mb-1" style={{ color: "var(--text-primary)" }}>
            Same item across all warehouses
          </h4>
          <div className="text-[11px] mb-2" style={{ color: "var(--text-muted)" }}>
            Shows whether this is a national supply problem or one DC.
          </div>
          <DataTable columns={columns} rows={data.rows} rowKey={(r) => r.wh} />
        </>
      )}
    </div>
  );
